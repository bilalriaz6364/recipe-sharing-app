...
export default function Profile() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-4 sm:py-8 px-2 sm:px-4 space-y-8">
      {/* Profile Header Box */}
      <div className="glass-panel p-4 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-800 shadow-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 blur-[80px] pointer-events-none" />

        <div className="flex flex-col sm:flex-row items-center sm:items-center gap-4 sm:gap-5 w-full sm:w-auto text-center sm:text-left">
          ...
          <div className="flex flex-col items-center gap-1.5 shrink-0">
            <div 
              ...
              className={`relative group w-20 h-20 sm:w-24 sm:h-24 rounded-2xl sm:rounded-3xl bg-gradient-to-tr from-emerald-500 via-emerald-400 to-amber-400 p-0.5 shadow-xl shadow-emerald-500/20 cursor-pointer overflow-hidden transition-all duration-300 hover:scale-105 ${
            ...
            >
              ...
              <div className="w-full h-full bg-slate-950 rounded-[18px] sm:rounded-[22px] overflow-hidden flex items-center justify-center relative">
                {uploadingAvatar ? (
                  ...
                  <span className="text-gradient-emerald-gold font-black text-2xl sm:text-3xl">
                ...
                <div className="absolute inset-0 bg-slate-950/75 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center text-emerald-400 transition-opacity duration-300 z-10">
                  ...
                  <span className="text-[8px] sm:text-[9px] font-black uppercase text-slate-100 tracking-wider">Change Pic</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-1 min-w-0 flex-1">
            {editingName ? (
              <div className="flex items-center justify-center sm:justify-start gap-2">
                <input
                  ...
                  className="bg-slate-950 border border-emerald-500 rounded-xl px-2.5 py-1 text-white text-xs sm:text-base focus:outline-none w-full max-w-[180px]"
                />
                <button
                  onClick={handleUpdateProfile}
                  className="p-1.5 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-bold rounded-xl transition shrink-0"
                >
              ...
              <div className="flex items-center justify-center sm:justify-start gap-2">
                <h1 className="text-xl sm:text-3xl font-black text-white truncate max-w-[220px] sm:max-w-none">
                  {profile.username || user?.email?.split('@')[0]}
                </h1>
                <button
                  onClick={() => setEditingName(true)}
                  className="text-slate-400 hover:text-emerald-400 transition p-1 shrink-0"
                  title="Edit Username"
                >
            ...
            <p className="text-slate-400 text-xs font-semibold truncate">{user?.email}</p>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 w-full sm:w-auto px-4 py-2.5 bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 rounded-2xl font-bold text-xs transition"
        >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 border-b border-slate-800 pb-3">
        <div className="flex gap-2 sm:gap-4 flex-wrap">
          <button
            ...
            className={`flex items-center gap-1.5 pb-2 font-extrabold text-[11px] sm:text-sm transition border-b-2 ${
              activeTab === 'my-recipes'
                ? 'border-emerald-500 text-emerald-400'
                : 'border-transparent text-slate-400 hover:text-slate-200'
            }`}
          >
            <ChefHat className="w-4 h-4 shrink-0" /> My Recipes ({myRecipes.length})
          </button>
          ...
          >
            <Bookmark className="w-4 h-4 shrink-0" /> Favorites ({favorites.length})
          </button>
        </div>

        <Link
          to="/add-recipe"
          className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-slate-950 rounded-xl text-xs font-black transition shadow-md text-center"
        >
          + Create New Recipe
        </Link>
      </div>
