...
export default function Home() {
  ...
  return (
    <div className="space-y-6 sm:space-y-10 py-3 sm:py-8">
      {/* Hero Banner */}
      <div className="relative text-center space-y-4 sm:space-y-5 py-6 sm:py-10 px-3.5 sm:px-4 rounded-2xl sm:rounded-3xl glass-panel border border-slate-800/80 shadow-2xl overflow-hidden">
        {/* Background glow ornaments */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 sm:w-96 h-72 sm:h-96 bg-emerald-500/10 blur-[100px] rounded-full pointer-events-none" />
        <div className="absolute top-1/4 right-10 w-32 sm:w-40 h-32 sm:h-40 bg-amber-500/10 blur-[60px] rounded-full pointer-events-none" />

        <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 py-1 sm:px-4 sm:py-1.5 rounded-full bg-slate-900/90 border border-emerald-500/30 text-emerald-400 text-[10px] sm:text-xs font-bold shadow-inner max-w-full">
          <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-pulse shrink-0" />
          <span className="truncate">The Precious Culinary Sharing Hub</span>
        </div>

        <h1 className="text-2xl sm:text-4xl md:text-6xl font-black tracking-tight leading-tight max-w-3xl mx-auto">
          Discover & Share <br className="hidden sm:inline" />
        ...
        {/* Feature Highlights */}
        <div className="pt-2 sm:pt-4 flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-[11px] sm:text-xs font-semibold text-slate-300">
